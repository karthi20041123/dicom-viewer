...
function DicomViewer({ files }) {
    ...
    try {
      ...
      if (pixelSpacingArray[0] === 1 && pixelSpacingArray[1] === 1) {
        try {
          ...
          if (imagerPixelSpacing) {
            const values = imagerPixelSpacing
              .split("\\")
              .map((v) => parseFloat(v.trim()));
            if (
              values.length >= 2 &&
      ...
      try {
        metadata.studyDescription =
          image.data?.string?.("x00081030") || "Unknown Study";
        metadata.seriesDescription =
          image.data?.string?.("x0008103e") || "Unknown Series";
        metadata.patientName =
          image.data?.string?.("x00100010") || "Unknown Patient";
        ...
        metadata.manufacturer =
          image.data?.string?.("x00080070") || "Unknown";
        metadata.institutionName =
          image.data?.string?.("x00080080") || "Unknown";
        ...
        .then((image) => {
          ...
          if (i === 0) {
            ...
            if (
              ...
              console.warn(
                "Pixel spacing adjusted for anatomical validation (eyeball ~24mm)"
              );
              const scaleFactor =
                24 /
                Math.max(
    ...
    return () => {
      for (let i = 0; i < viewerCount; i++) {
        ...
        if (element) {
          ...
          element.removeEventListener(cornerstone.EVENTS.NEW_IMAGE, () =>
            handleViewportChange(i)
          );
        }
      }
  ...
  const renderViewers = () => {
    ...
    const protocolPanel = (
      <div className="protocol-panel" draggable>
        ...
        {files.map((file, index) => (
          <div
            key={index}
            className={`protocol-panel-item ${index === currentIndex ? "active" : ""}`}
            draggable
            onDragStart={(e) => {
    ...
    return (
      <>
        {protocolPanel}
        <div className="viewers-container">{viewers}</div>
      </>
    );
  };
  ...
  return (
    <div className="dicom-viewer-container">
      ...
      {/* Combined container for toolbar and viewers */}
      ...
      {isImageLoaded && (
        <div className="brightness-contrast-container">
          <Typography sx={{ color: "#020079", fontWeight: "bold" }}>
            Brightness
          </Typography>
          <Slider
            value={brightness}
        ...
        onClick={() => {
          setIsMeasuring((prev) => {
            ...
            if (newIsMeasuring) {
              ...
              for (let i = 0; i < viewerCount; i++) {
                const element =
                  i === 0 ? viewerRef.current : viewerRefs.current[i]?.current;
                if (element) {
                  let isEnabled = false;
                  try {
                    if (cornerstone.getEnabledElement(element)) isEnabled = true;
      ...
      {isImageLoaded && pixelSpacing && (
        <Typography
          variant="caption"
          className="info-text info-text-pixel-spacing"
        >
          Pixel Spacing: {pixelSpacing[0]?.toFixed(3)} ×{" "}
          {pixelSpacing[1]?.toFixed(3)} mm/pixel
        </Typography>
      )}
      ...
      {isImageLoaded && (
        <Typography
          variant="caption"
          className="info-text info-text-scale"
        >
      ...
      {isImageLoaded && imageMetadata && (
        <Typography
          variant="caption"
          className="info-text info-text-metadata"
        >
